...
fn analyzse_dir(dir: &str) -> io::Result<Vec<File>> {
    ...
    for file in files {
        ...
        if metadata.is_dir() {
            data.append(&mut analyzse_dir(&format!("{}", file.path().display()))?);
        } else {
            let f = File { name: String::from(file.path().file_name().unwrap().to_str().unwrap()), size: metadata.len(), is_dir: metadata.is_dir() };
            data.push(f);
        }
    }
    return Ok(data);
